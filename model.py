...
def build_fpn_keypoint_graph(rois, feature_maps,
    ...
    for i in range(8):
        ...
        x = KL.TimeDistributed(BatchNorm(axis=3),  # channels层面正则化
    ...
    return x  # shape: batch_size, num_roi, num_keypoint, 56*56
...
def load_image_gt_keypoints(dataset, config, image_id, augment=True,
    ...
    image, window, scale, padding = utils.resize_image(
        ...
        padding=config.IMAGE_PADDING)
    # mask = utils.resize_mask(mask, scale, padding)
    keypoints = utils.resize_keypoints(keypoints, image.shape[:2], scale, padding)  # resize后keypoints
    ...
    # bbox = utils.extract_bboxes(mask)
    bbox = utils.extract_keypoint_bboxes(keypoints, image.shape[:2])
